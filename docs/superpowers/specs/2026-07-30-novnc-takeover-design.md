...
面板 (8790, 已鉴权)
  ...
  ├─ GET  /vnc/{path}   (鉴权文件路由)  noVNC 前端静态资源（鉴权 + 防穿越）
  └─ WS   /vnc/websockify               鉴权后直接桥接到 127.0.0.1:5900（VNC 裸 TCP）

接管会话（后台，单例）：
  Xvfb :100
    → Camoufox(headless=False, DISPLAY=:100, 注入 sessionKey + 账号 proxy)
    → goto https://claude.ai   （已登录态）
    → x11vnc  -display :100 -localhost -forever -shared -rfbport 5900 -nopw
```

> **不用 websockify**：noVNC 前端要的「WebSocket ↔ VNC TCP」桥直接由 FastAPI 的
> `/vnc/websockify` 端点实现（把 WS 二进制帧和到 `127.0.0.1:5900` 的 asyncio TCP
> 连接双向对拷）。省掉一个常驻进程和一个端口，且桥这一跳天然经过面板鉴权。
...
固定约定（全部只绑 `127.0.0.1`）：display `:100`、VNC `5900`。因为是单例接管，固定端口不会冲突。
...
- **`start(account)`**（account 含 email / sessionKey / proxy）：
  ...
  5. 起 `x11vnc -display :100 -localhost -forever -shared -rfbport 5900 -nopw -quiet`。
- **`stop()`**：反序拆掉 x11vnc → Camoufox → Xvfb 全部子进程，重置状态。
...
**可测试性**：把「起 Xvfb / x11vnc」两个外部子进程抽象成一个可注入的 `ProcessLauncher` 接口（默认实现用 `subprocess.Popen`）；开浏览器抽象成可注入的 `browser_fn`（默认实现 `open_takeover_browser`，返回一个带 `.close()` 的句柄）。单测注入假实现，不真的拉起 X 服务或浏览器。

### 2. noVNC 反代进面板

- **Dockerfile**：`apt-get install` 增加 `x11vnc`、`novnc`（noVNC 提供 `vnc.html` + 静态前端，装在 `/usr/share/novnc`）。
- **静态资源**：noVNC 前端用一个鉴权文件路由 `GET /vnc/{path}` 提供（`require_auth` + 防路径穿越，仿现有 `/runs/{run_id}/{filename}` 工件路由；不能用裸 `StaticFiles`，mount 不继承 `Depends`）。
- **WebSocket 桥接**：新增 FastAPI WebSocket 端点 `/vnc/websockify`，先校验面板 cookie（复用 `auth.verify_token`），通过后把该 WS 的二进制帧与一条到 `127.0.0.1:5900` 的 asyncio TCP 连接双向对拷。
- **绑定**：x11vnc 只绑 `127.0.0.1`；容器**不新增 EXPOSE / ports**，唯一入口是面板 8790 的鉴权反代。

> 静态和 ws 都走面板的鉴权路由，保证两条路径都经过面板密码校验；VNC 5900 只在容器内 localhost 可达。
...
| 未登录面板（含 `/vnc/*`、WS） | `401` |
| Xvfb/x11vnc/浏览器 任一起不来 | `start` 内部回滚已起的子进程/浏览器，返回 `500` 并把原因写日志 |
...
2. **全 localhost 绑定**：x11vnc `-localhost`，容器内进程互访，不落到宿主网卡。
...
- **WS 桥鉴权**：无 cookie（或密码未设）连 `/vnc/websockify` 被 `close(1008)` 拒。
- 真 Xvfb/x11vnc/浏览器不进 CI（self-hosted runner 也不跑），靠可注入接口隔离。

## 交付边界

- 新增：`server/takeover.py`、`/vnc/{path}` 静态与 `/vnc/websockify` WS 路由、账号列表「接管」按钮与状态区、`takeover` 配置段。
- 修改：`Dockerfile`（装 x11vnc/novnc）、`claude_register/browser.py`（抽出 `build_camoufox_kwargs` + 新增 `open_takeover_browser`）、`server/app.py`（挂路由）、`server/deps.py`（装配 TakeoverManager）、`config.example.yaml`/`config_store`（新增配置段）、`web/src/api.js` + `web/src/pages/Dashboard.jsx`（UI）。
